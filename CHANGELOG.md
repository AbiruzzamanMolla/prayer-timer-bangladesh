...
- **Congregational Prayer Tracking:** Added configurable offsets to define your local Congregational Prayer times.
- **Congregational Notifications:** The VS Code native host will now notify you X minutes before a Congregational Prayer begins.
- **Webview Countdown Panel:** A visual countdown banner will now appear in the webview if the current time is within 5 minutes of a scheduled Congregational Prayer.
...
- **Complete Architecture Overhaul:** Rebuilt the extension as a React Webview for a beautiful, responsive, and custom-designed UI Dashboard.
- **Autonomous Status Bar Application:** Built a background native extension loop that parses `Adhan-js` natively. The Status Bar ticks down automatically `(Fajr in 6h 30m)` without ever needing to open a UI panel.
- **Rich Status Bar Tooltips:** Hovering over the status bar automatically shows the entire day's prayer schedule dynamically calculated natively.
- **Instantaneous Start:** Standardized the default startup configuration to target Dhaka, Bangladesh using Muslim World League/Hanafi. The application immediately tracks time seamlessly on install!
- **Integrated `Adhan-js`:** Completely replaced external API endpoints with algorithmic, offline calculating equations to ensure absolute speed, privacy, and constant uptime.
- Added advanced settings for calculation methodology, High Latitude Rules, Asr configurations, and Midnight calculation overrides.

### Fixed

- **Status Bar Night Roll-over Bug:** Fixed an issue where the background timer would improperly instantly skip the Isha countdown and proceed to counting Fajr time. It now accurately tracks the true clock.
- **Status Bar Ramadan Mode:** Replicated the React Webview's Ramadan mode target in the extension host. When Ramadan is toggled on, the Status Bar background timer will now show "Sehri" appropriately overnight.
- **Status Bar Fluctuations:** Un-tied the Webview clock from the generic Extension clock! Changing the location in the UI now passes the details via `saveSettings` to `globalState` ensuring there is a strict single source of truth for calculations.

### Changed

- **Breaking Change:** The system now stores all preferences in `vscode.globalState` rather than `.json` configurations or relying on external API connections. Users who upgrade will see the new UI instantly on click, but may have to reconfigure their location if they are outside of the default location.
- Redesigned visual branding and icons for the marketplace.
